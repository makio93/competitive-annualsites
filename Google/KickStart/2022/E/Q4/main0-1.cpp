    ...
    rep3(i0, 1, t0+1) {
        ...
        while (!pq.empty()) {
            ...
            rep(ii, 5) {
                ...
                int pid = psearch(ny, nx), nplst = plst;
                if (pid!=-1 && !((plst>>pid)&1)) {
                    nplst |= 1 << pid;
                    ndi = add(mi+1, c[pid], ndi);
                }
                if (dist[ny][nx][mi+1][nplst] <= ndi) continue;
                dist[ny][nx][mi+1][nplst] = ndi;
                if (mi+1 < m) pq.emplace(ndi, ny, nx, mi+1, nplst);
            }
        }
        ll res = LINF;
